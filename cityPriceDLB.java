...
public class cityPriceDLB 
{
    ...
    void createRoot(String word)
    {
        ...
        tempRoot.child = new Node('$', null, null, new PriceHeap()); // $ is terminator char
    }
    ...
    void recursiveAdd(StringBuilder word, Node root, Node parent)
    {
        ...
        if(word.length()==0)
        {
            tempRoot.child = new Node('$', null, null, new PriceHeap());   // end of recursion
            return;         
        }
        else
        {
    ...
    void createChildren(StringBuilder word, Node root)
    {
        ...
        {
            tempRoot.child = new Node('$', null, null, new PriceHeap());  
            return;         
        }
     ...
     * @return predictions
     */
    public PriceHeap get(String word)
    {
        Node end = findEnd(word, root, 0);
    ...
    private class Node
    {
        ...
        Node child;
        Node sibling;
        public Node(char value, Node nextChild, Node nextSibling, PriceHeap prices)
        {
            this.val = value;
